...
The kubebuilder-generated `config/` tree is the canonical manifest layout. The Helm chart at `charts/redis-operator/` MUST wrap those manifests for parameterised deployment so both packaging paths stay functionally aligned.
