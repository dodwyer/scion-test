...
- [x] Confirm all five field validation rules match the spec
- [x] Confirm exit codes 0 / 1 / 2 are returned correctly in each scenario
- [x] Confirm error messages include field paths in `ERROR: <field.path>: <reason>` format
- [x] Confirm unit tests cover the valid case and every invalid-field scenario individually
- [x] Confirm example YAMLs are syntactically valid and illustrate the documented rules
- [x] Confirm README usage section shows install, invocation, and example output
- [x] Confirm no admission webhook, CRD schema, or runtime cluster code was introduced
